...
/*     getOpotholParamVisitNo(connection,branch_id, seq_type) {
    ...
    getOpotholParamVisitNo(connection, data, seq_type) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                var custQuery = `SELECT max(visit_no) AS visit_no FROM ${process.env.WRITE_DB_DATABASE}.Swastha_Opthol_Visit_Detail WHERE org_id='${data.org_id}' 
    ...
    createOptholParamData(connection, set_opthol_data) {
        return new Promise(async(resolve, reject) => {
            try {
                if(connection == null ) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!.", developerMessage:"DB Connection Failed!." };
                ...
                await connection.query(`INSERT INTO ${process.env.WRITE_DB_DATABASE}.Swastha_Opthol_Visit_Detail SET ?`, set_opthol_data);
    ...
    getOptholParamDetail(connection, org_id, branch_id, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                var custQuery = `SELECT * FROM ${process.env.WRITE_DB_DATABASE}.Swastha_Opthol_Visit_Detail WHERE org_id='${org_id}' 
                AND branch_id='${branch_id}' AND visit_no='${query.filter.visit_no}' AND patient_id='${query.filter.patient_id}'`;
                debug("getOptholParamDetailData", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    debug('Sorry,getOptholParamDetailData!.', queryres);
